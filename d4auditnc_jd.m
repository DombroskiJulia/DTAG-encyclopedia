...
%     or
%     R = d3audit(recdir,tag,tcue,R)   % d3 tags
%     or
...
%
%Julia Dombroski, April 13, 2020 
%Notes
%Make sure the nc file is in the work directory
%Also check the nc name. Standard in this function is sens5. If different
%change line 133 acordingly. 
%To Adjust spetrogram frequency range change ylim is line 228


NS = 60;          % number of seconds to display
BL = 2024;         % specgram (fft) block size
CLIM = [-100 10] ;   % color axis limits in dB for specgram
...
tcue = [] ; RES = [] ; recdir = [] ; prefix = [] ;

if nargin<5,
   [varargin{nargin+1:5}] = deal([]) ;
end
...
if exist('P','var'),
   [p,fs] = sens2var(P) ;
    ...
    fprintf('nc file not found - continuing without\n Check if nc is the workdirectory/n Check if nc is sens5')
end
...
while 1,
   ...
   kk = 1:5:length(xx) ;
   axes(AXm), plot(tcue+kk/afs,xx(kk),'k') ; grid %plots the energy by time plot
   ...
   axis xy, grid ;
   %ylim([0 8]) %sets y axis limit for spectrogram;  
   if ~isempty(p),
      set(AXs,'XTickLabel',[]) ;
   else
      xlabel('Time, s')
