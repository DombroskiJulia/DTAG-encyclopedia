function [bton,btoff] = bottom_70percent(p,fs,T)
...
for dn=1:ndives  %run for the number of dives  
   ...
   bton(dn) = ds(dn)+find(pdive>0.7*pmax(dn),1)-1 ; % start of 'bottom'
   btoff(dn) = ds(dn)+find(pdive>0.7*pmax(dn),1,'last') ; % end of 'bottom'
end


bton=bton./fs; % time in seconds
btoff=btoff./fs;
