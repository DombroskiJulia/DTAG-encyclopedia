function    [lat,lon,h,t,N] = gps_posns(Obs,pos,tc,THR,kproc)
%
%    [lat,lon,h,t,N] = gps_posns(Obs,pos,tc,[THR,kproc])
...
for k=1:length(Obs),  % estimate a position for each grab
   ...
   N(k,:) = [nsv mean(p(p(:,2)>THR,2)) pp([end-1 end-2])] ;
   T(k,6) = T(k,6)+pp(end-1) ;
   t(k) = datenum(T(k,:)) ;
   plot(lon(k),lat(k),'b.'),hold on,drawnow
   ...
   if pp(end-2)<SRCH_THR && abs(h(k))<200,    % if positional accuracy is good update time offset
   ...
   if rem(k,10)==0,
      save _gpsposn_part.mat lat lon h t N
   end
end
return
