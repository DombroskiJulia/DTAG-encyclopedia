...
%     Display echogram and allow selection of echo sequences from a dtag3
%     recording on an echolocating animal.
...
%        The tool will choose appropriate settings for some frequently tagged
%        species according to the prefix. Edit the default settings for
%        other species.
...
%     CL is a vector of click cues e.g., from d3findclicks or
%        d3findallclicks.
%     intervl = [left right] are the times in seconds to display with
%        respect to each click. Default is [-0.001 0.025].
%     DD is the output of a previous session and can be passed as an
%        input argument to allow multiple work sessions.
%
%     Returns:
%        DD is a cell array containing a matrix for each echo sequence identified.
%        Each matrix in DD contains the following columns:
%        1  click_cue
...
%
%     Echograms will be displayed 20s at a time (or another amount if a
%     two-element cue is specified). Pressing the left mouse button near an
%     echo will open a new figure showing the waveform received by the tag
%     at the time of the echo and indicating the peak time for the echo.
%     Pressing the left button again will select the peak time as an echo
%     point. Alternatively, type x to quit this window and return to the
%     echogram, or s to select the time at the mouse position for the echo.
%     Echoes selected in this way form an echo sequence. In the echogram 
%     window, you can interact with the echogram using the following keystrokes:
%
%        e  edit an existing echo sequence
%        a  accept and close an existing sequence
%        r  delete all echoes in an open sequence
%        z  automatically detect echoes close to the line between two 
%           mouse positions. The first position is where the mouse is when
%           z is typed. Then move to the second position and type z again.
%        b  go back to previous 20s interval
%        f  go forward to next 20s interval
%        q  quit
%
%     Note: if the function fails for whatever reason, a backup file of the
%        currently selected echoes is made in the Matlab working directory
%        with name echotool_Recover.mat. Load this file into Matlab to
%        continue working after a fail.
%
