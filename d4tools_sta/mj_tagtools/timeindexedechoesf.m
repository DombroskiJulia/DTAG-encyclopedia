function    [scl,RR] = timeindexedechoesf(fname,cue,CL,intervl,df,tz,noedit)
%
%     [scl,RR] = timeindexedechoesf(fname,cue,CL,intervl,df,tz,noedit)
%     Display echogram with a time axis rather than a click axis.
%     fname is the full file name
...
%   f_env_ls = envelope detector signal bandpass filter 
cax = [-100 -30] ;         
f_env = [115000 150000] ;
...
elseif length(cue)==2,
   ...
   len = 10 ;        % default length of segment to analyze in seconds
end
...
[b a] = butter(6,f_env/(fs/2)) ;

x = wavread(fname,1+round(fs*(cue+[0 len+0.1+intervl(2)]))) ;
