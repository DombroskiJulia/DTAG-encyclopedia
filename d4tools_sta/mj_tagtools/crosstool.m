function    [D,Rd]=crosstool(tag2,cl1,cue,offset,D,DMON)
%
%     D=crosstool(rtag,scl,cue,offset,[D,DMON])
...
while 1,
   ...
   while next==1,
       ...
       if next,
          ...
          if size(x2,2)==1 & nargin>5 & ~isempty(DMON),
             [x2,cleanh] = dmoncleanup(x2,0.0001,[],cleanh) ;
          end
          cl = cl-cue ;
   ...
   if next,
      ...
      if next==2 & exist('RR','var') & nargout>1,
         Rd.R = RR ;
         Rd.X = dk ;
         Rd.Y = 1:ncl ;
      end
...
if button==3 | button=='x',
   xr = NaN ;                 % delete point
elseif isempty(button) || button=='m',
   xr = mx ;
elseif yr<0,
   xr = pt ;
end
