...
%     q - quit
%     c - change hydrophone channel
%     l - change low frequency cutoff
%     h - change high frequency cutoff
...
CH = 0 ;
ORD = 6 ;               % filter order (number of poles)
%BLANKING = 5e-3 ;      % for sperm whales
...
while 1,
   ...
   while done==0,
      ...
      elseif button=='+',
         set(gca,'YLim',[0 0.5*get(gca,'YLim')*[0;1]]) ;
         
      elseif button=='-',
         set(gca,'YLim',[0 2*get(gca,'YLim')*[0;1]]) ;
         
      elseif button=='c',
         CH = CH+1 ;
         if CH>size(x,2),
            CH = 0 ;
            fprintf(' Using all channels\n') ;
         else
      ...
      elseif button=='l',
         FBP(1) = round(input(sprintf(' Change low cutoff from %d Hz to... ',FBP(1)))) ;
         fprintf('\n') ;
         [b a] = butter(ORD,FBP/(afs/2)) ;
         done = 1 ; FORCE = 1 ;
         
      elseif button=='h',
         FBP(2) = round(input(sprintf(' Change high cutoff from %d Hz to... ',FBP(2)))) ;
         fprintf('\n') ;
         [b a] = butter(ORD,FBP/(afs/2)) ;
         done = 1 ; FORCE = 1 ;
