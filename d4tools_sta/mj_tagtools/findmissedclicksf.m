function    CL = findmissedclicksf(fname,CL,FBP,TH,cue)
%
%    C = findmissedclicksf(fname,C,[FBP,TH,cue])
...
WIN = 1000 ;             % show +-WIN samples in click select window
SWIN = 0.5e-3 ;
MARGIN = 0.5 ;
RELTH = 0.1 ;
MAXICI = 1 ;            % maximum ICI to display
MAXSHOW = 2 ;           % maximum gap to show
...
while 1,
   ...
   x = wavread(fname,1+round(afs*[st ed])) ;
   xf = filter(b,a,x(:,1)) ;
   xx = hilbenv(xf) ;
