function    cl = sortclicks_trials(x,fs,cl)
%
%     cl = sortclicks_trials(x,fs)
%        Generate click list and then allow interactive sorting
...
BLANKING = 0.0003 ;      % blanking time in ms
...
L = 20*log10(cl(:,2)) ;
CAX = max(L)+[-60 0] ;
opts.blanking = BLANKING; opts.fh=[]; opts.env = 1;
done = 0 ;
figure(1),clf
...
while(done<2),
   ...
   while ~done,
      ...
      switch button,
         ...
         case 'e',
            figure(2),clf
            [X,cc]=extractcues(x,cl(cl(:,3)==0,1)*fs,round(fs*[-0.0005 ECHOGRAM_EXTENT]));
         ...
         case 3
            ...
            kxx = max(1,round(fs*(cl(kl,1)+BLANKING))):min(round(fs*cl(ku,1)),length(x)) ;
            xx = x(kxx) ;        
            % find all clicks in segment that are above gy
            opts.protocol = 'first';
