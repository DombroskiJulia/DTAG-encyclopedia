...
k = loadprh(tag,0,'p','P','fs','A','J') ;           % read p and fs from the sensor file
if k==0,
...
if ~isempty(p),
   ...
   AXj = axes('position',[0.11, 0.54,0.78, 0.15]);
   AXs = axes('position',[0.11,0.34,0.78,0.15]) ;
...
while 1,
   ...
   x(isnan(x)) = 0 ;
   [B F T] = specgram(x(:,CH),BL,afs,hamming(BL),BL/2) ; colormap jet;
   xx = filter(pp,[1 -(1-pp)],abs(filter(bh,ah,x(:,CH)))) ;

    ks = kb + round(tcue*fs) ;
   %kk = 1:5:length(xx) ;
   %axes(AXm), plot(tcue+kk/afs,xx(kk),'k') ; grid
   axes(AXm),plot(ks/fs,A(ks,1:3)),grid
 ...
 ks = kb + round(tcue*fs) ;
   %kk = 1:5:length(xx) ;
   %axes(AXm), plot(tcue+kk/afs,xx(kk),'k') ; grid
   axes(AXj),plot(ks/fs,J(ks,1)),grid
   %set(AXj,'XAxisLocation','top') ;
   yl = get(gca,'YLim') ;
   %yl(2) = min([yl(2) MAXYONCLICKDISPLAY]) ;
   axis([tcue tcue+NS yl]) ;
   ylabel 'Jerk ms^-^3'
