function [y,Fs,bits,opt_ck] = wavread_ext(varargin)
...
if opt_ck.fmt.wFormatTag == 1 || opt_ck.fmt.wFormatTag == 3 || opt_ck.fmt.wFormatTag == 65534,
...
msg = '';
err_msg = getString(message('MATLAB:audiovideo:wavread:ErrorReadingChunk','<cue-ck>'));
...
for cue = 1:opt_ck.cues
    ...
    opt_ck.cue_ID = fread(fid,1,'uint32');
    opt_ck.cue_Position = fread(fid,1,'uint32');
    opt_ck.cue_ckID = fread(fid,1,'uint32');
    opt_ck.cue_ckstart = fread(fid,1,'uint32');
    opt_ck.cue_bkstart = fread(fid,1,'uint32');
    opt_ck.cue_sampleoffset = fread(fid,1,'uint32');
end
...
elseif (BytesPerSample == 4),
    ...
    elseif (wavefmt.wFormatTag == 65534) % 32-bit 16.8 float (type 1 - 32-bit)
        dtype = 'int32'; %signed 32-bit    
    end
